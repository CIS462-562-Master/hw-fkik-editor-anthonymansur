...
bool mat3::ToEulerAngles(RotOrder order, vec3& angleRad) const
{
    ...
    case YXZ:
        ...
        // NOTE: did not implement edge cases
        angleRad = vec3(0.0, 0.0, 0.0);
        double fi = asin(-mM[1][2]);
        double psi = atan2(mM[1][1], mM[1][0]);
        double theta = atan2(mM[2][2], mM[0][2]);
...
mat3 mat3::FromEulerAngles(RotOrder order, const vec3& anglesRad)
{
    ...
    case YXZ:
        ...
        double theta = anglesRad[VY];
        double fi = anglesRad[VX];
        double psi = anglesRad[VZ];
        vec3 v1 = vec3(cos(theta), 0, sin(theta));
        vec3 v2 = vec3(0, 1, 0);
        vec3 v3 = vec3(-sin(theta), 0, cos(theta));
        mat3 rotY = mat3(v1, v2, v3);

        v1 = vec3(1, 0, 0);
        v2 = vec3(0, cos(fi), -sin(fi));
        v3 = vec3(0, sin(fi), cos(fi));
        mat3 rotX = mat3(v1, v2, v3);

        v1 = vec3(cos(psi), -sin(psi), 0);
        v2 = vec3(sin(psi), cos(psi), 0);
        v3 = vec3(0, 0, 1);
...
void quat::FromRotation(const mat3& rot)
{
    ...
    double q0 = sqrt((1 + rot[0][0] + rot[1][1] + rot[2][2]) / 4.0); // w
    double q1 = sqrt((1 + rot[0][0] - rot[1][1] - rot[2][2]) / 4.0); // x
    double q2 = sqrt((1 - rot[0][0] + rot[1][1] - rot[2][2]) / 4.0); // y
    double q3 = sqrt((1 - rot[0][0] - rot[1][1] + rot[2][2]) / 4.0); // z
    ...
    if (abs(q1) == max) {
        q0 = (rot[2][1] - rot[1][2]) / (4.0 * q1);
        q2 = (rot[0][1] + rot[1][0]) / (4.0 * q1);
        q3 = (rot[0][2] + rot[2][0]) / (4.0 * q1);
    }

    if (abs(q2) == max) {
        q0 = (rot[0][2] - rot[2][0]) / (4.0 * q2);
        q1 = (rot[0][1] + rot[1][0]) / (4.0 * q2);
        q3 = (rot[1][2] + rot[2][1]) / (4.0 * q2);
    }

    if (abs(q3) == max) {
        q0 = (rot[1][0] - rot[0][1]) / (4.0 * q3);
        q1 = (rot[0][2] + rot[2][0]) / (4.0 * q3);
        q2 = (rot[1][2] + rot[2][1]) / (4.0 * q3);
    }
...
quat quat::Slerp(const quat& q0, const quat& q1, double u)
{
    ...
    double omega = acos(Dot(q0, q1) / (q0.Length() * q1.Length()));
    if (abs(omega) < EPSILON) {
        return q0;
    }
    double a = sin((1 - u) * omega) / sin(omega);
    double b = sin(u * omega) / sin(omega);
    return a * q0 + b * q1;
}
...
void quat::ToAxisAngle(vec3& axis, double& angleRad) const
{
    ...
    angleRad = acos(mQ[VW]) * 2.0;

    if (abs(angleRad) < EPSILON) {
        axis[0] = 0; axis[1] = 0; axis[2] = 0;
        return;
    }
    axis[0] = mQ[VX] / sin(angleRad / 2.0);
    axis[1] = mQ[VY] / sin(angleRad / 2.0);
    axis[2] = mQ[VZ] / sin(angleRad / 2.0);
}
...
mat3 quat::ToRotation() const
{
    ...
    double z = mQ[VZ];
    double s = mQ[VW];
    m[0][0] = 1 - 2 * pow(y, 2.0) - 2 * pow(z, 2.0);
    m[0][1] = 2 * x * y - 2 * s * z;
    m[0][2] = 2 * x * z + 2 * s * y;
    m[1][0] = 2 * x * y + 2 * s * z;
    m[1][1] = 1 - 2 * pow(x, 2.0) - 2 * pow(z, 2.0);
    m[1][2] = 2 * y * z - 2 * s * x;
    m[2][0] = 2 * x * z - 2 * s * y;
    m[2][1] = 2 * y * z + 2 * s * x;
    m[2][2] = 1 - 2 * pow(x, 2.0) - 2 * pow(y, 2.0);
    return m;
}
